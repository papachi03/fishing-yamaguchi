# YouTube概要欄 コピペリスト（釣り動画・36本）

作成：2026-09-13

> ## ✅ 2026-09-13 全36本 反映済み（読み込み直しで最終確認）
> - **上位3本**（gc1c6fg2hU0 / O71B6QOn4qs / Y3suaT2xEkk）…ぱっぱが先に貼っていた。**上の1行＋下の枠の2か所**・動画IDの印なし。
>   → GA4で `utm_content` が **(not set)** の `youtube / description` は、この3本の合計
> - **残り33本** … クロロが先頭に1行を貼り、ぱっぱが保存。**動画IDの印あり**（1か所のみ）
> - 作業のしかた：クロロがブラウザで貼る→ページ内で「元の説明文が1文字も欠けていない」を照合→ぱっぱが「保存」
> - **このリストの作業はもう不要。新作を上げたときだけ、その動画の1行を足す**
> - **固定コメント（2026-09-13 公開ページで確認）**：上位3本（6.9万／5.3万／2.9万回）は**付いている**（リンク入り・`utm_medium=comment`）。
>   4本目以降は付けない方針（例：K7tnITvZeYo は2,494回。★は当初リストの並び順で機械的に付けたもので再生数基準ではなかった）
> - ⚠️ 未確認：`Prd-FpgvXmk` のYouTube上のタイトルが「堤防釣りで春のコウイカはどこまで釣れる？【後編】」で、
>   サイトのJOURNALに記録した「このエギはホンモノでした」と違う（照合ミスか改題か、ぱっぱに確認待ち）

## やり方（1本30秒）

1. 「Studioで開く」を押す（YouTube Studioの編集画面が開く）
2. **説明欄の一番上**に、その動画の「貼る1行」をコピペ
3. 右上の「保存」

- 行末の `utm_content=` の後ろが**動画ごとに違う**。GA4で「どの動画から来たか」を見分けるための印なので、**必ずその動画の行を使う**（使い回さない）
- 既存の説明文は消さない。一番上に1行足すだけ
- ショート動画には貼らない（YouTubeの仕様でリンクが押せない）
- **固定コメントは再生数が多い上位3本だけ**（済み）。再生数の少ない動画には付けなくてよい
- 並びは「再生数が分かっている人気上位 → サイトのJOURNALに載せている動画 → 最新の釣り動画」の順。**時間が無ければ上から**

## GA4での見方

レポート → 集客 → トラフィック獲得 → 表の見出しの「＋」から **「セッションの手動広告コンテンツ」** を追加。
動画IDごとに訪問数が並ぶ。IDとタイトルの対応はこのリストで引く。

---

### 1. 【ヤリイカ】明かりはNGの常識を覆す新ヤリイカ釣法【灯火フィシング】

36:55 ／ ID `gc1c6fg2hU0` ／ 6.8万回 ／ 人気上位
...
19:36 ／ ID `O71B6QOn4qs` ／ 5.3万回 ／ 人気上位
...
13:44 ／ ID `Y3suaT2xEkk` ／ 2.9万回 ／ 人気上位
...
20:09 ／ ID `K7tnITvZeYo` ／ JOURNAL掲載
...
12:42 ／ ID `Prd-FpgvXmk` ／ JOURNAL掲載
...
16:18 ／ ID `l61yLE7CTgU` ／ JOURNAL掲載
...
26:29 ／ ID `ONXgL5oc10Q` ／ JOURNAL掲載
...
16:54 ／ ID `cbLq5JyL-Pg` ／ JOURNAL掲載
